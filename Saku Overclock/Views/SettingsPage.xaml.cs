...
public sealed partial class SettingsPage
{
    ...
    private readonly INotifyIconsService _notifyIcons = App.GetService<INotifyIconsService>(); // Настройки RTSS

    private readonly IKeyboardHotkeysService
        _hotkeysService = App.GetService<IKeyboardHotkeysService>(); // Горячие клавиши
    ...
    private void InitializePage()
    {
        try
        {
            AutoStartComboBox.SelectedIndex =
                _appSettings.AutostartType is > -1 and < 3 ? _appSettings.AutostartType : 0;
            ...
            EnableKeybindingsSettingGrid.CornerRadius =
                _appSettings.HotkeysEnabled ? new CornerRadius(15, 15, 0, 0) : new CornerRadius(15);
            if (!_appSettings.HotkeysEnabled) ((ScaleTransform)KeybindingsGrid.RenderTransform).ScaleX = 0.0;
    ...
    private void InitializeThemeSettings()
    {
        ...
        try
        {
            ...
            if (_themeSelectorService.Themes.Count == 0) return;
    ...
    {
        if (string.IsNullOrEmpty(advancedCode)) return;
    ...
    {
        Process.Start(new ProcessStartInfo("https://github.com/Erruar/Saku-Overclock/blob/master/EULA.md")
    ...
    {
        Process.Start(new ProcessStartInfo("https://github.com/Erruar/Saku-Overclock/blob/master/LICENSE.md")
            { UseShellExecute = true });
    }
    ...
    private void PivotSettings_Loaded(object sender, RoutedEventArgs e)
    {
        ...
        foreach (var item in items) item.Margin = new Thickness(0, 22, 0, 0);
    ...
    private void LoadTrayMonIconElements()
    {
        ...
        if (_notifyIcons.Elements.Count == 0) return;

        // Заполняем комбобокс
        foreach (var trayIcon in _notifyIcons.Elements)
            try
            {
    ...
    {
        if (_appSettings.NiIconsType < 0 || _appSettings.NiIconsType >= _notifyIcons.Elements.Count) return;

        var selectedIcon = _notifyIcons.Elements[_appSettings.NiIconsType];
    ...
    private void ShowTrayMonIconControls()
    {
        ...
        if (!showDetailedSettings) UpdateEnabledElementCornerRadius();
    }
    ...
    private async void AddTrayMonIcon_Click(object sender, object e)
    {
        try
        {
            ...
            try
            {
                if (_notifyIcons.Elements.Count != 0)
                    for (var element = 8; element < _notifyIcons.Elements.Count; element++)
                    {
                        var baseNiName = _notifyIcons.Elements[element].Name;
                        Color baseNiBackground; // Белый 
                        if (_notifyIcons.Elements[element].Color != "")
                            try
                            {
                                baseNiBackground = ParseColor(_notifyIcons.Elements[element].Color);
                            }
                            catch
                            {
                            ...
                            Content = new Grid
                            {
                                ...
                                Children =
                                {
                                    new Border
                                    {
                                        ...
                                        CornerRadius = new CornerRadius(17),
                                        Width = 430,
                                        Opacity = _notifyIcons.Elements[element].BgOpacity,
                                        VerticalAlignment = VerticalAlignment.Stretch,
                                        HorizontalAlignment = HorizontalAlignment.Stretch,
                        ...
                        if (element > 8) eachButton.Margin = new Thickness(0, 10, 0, 0);
                ...
                if (niLoaderPanel.Children.Count > 0) newNiIcon.Margin = new Thickness(0, 10, 0, 0);
                ...
                newNiIcon.Click += (_, _) =>
                {
                    ...
                    if (niIconSelectedComboBox.Items.Count >= 1) niIconSelectedComboBox.SelectedIndex = 0;
                    ...
                    niIconAddButtonSuccess.Click += (_, _) =>
                    {
                        if (niIconSelectedComboBox.SelectedIndex != -1)
                            try
                            {
                                _notifyIcons.Elements.Add(new NiIconsElements
                                {
    ...
    private void TrayMonIconElements_SelectionChanged(object? sender, SelectionChangedEventArgs? e)
    {
        ...
        if (_notifyIcons.Elements.Count != 0 && _appSettings.NiIconsType != -1)
        {
            ...
            IsTrayMonIconShowing.IsOn = _notifyIcons.Elements[_appSettings.NiIconsType].IsEnabled;
            SettingsNiEnabledElementGrid.CornerRadius = IsTrayMonIconShowing.IsOn
                ? new CornerRadius(0)
                : new CornerRadius(0, 0, 15, 15);

            if (!_notifyIcons.Elements[_appSettings.NiIconsType].IsEnabled)
            {
                NiIconStackPanel.Visibility = Visibility.Collapsed;
                SettingsNiContextMenu.Visibility = Visibility.Collapsed;
            }

            NiIconCombobox.SelectedIndex = _notifyIcons.Elements[_appSettings.NiIconsType].ContextMenuType;
            NiIconsColorPickerColorPicker.Color =
                ParseColor(_notifyIcons.Elements[_appSettings.NiIconsType].Color);
            SettingsNiGradientToggle.IsOn = _notifyIcons.Elements[_appSettings.NiIconsType].IsGradient;
            NiIconShapeCombobox.SelectedIndex = _notifyIcons.Elements[_appSettings.NiIconsType].IconShape;
            NiIconFontWeight.SelectedIndex = _notifyIcons.Elements[_appSettings.NiIconsType].FontWeight;
            SettingsNiFontsize.Value = _notifyIcons.Elements[_appSettings.NiIconsType].FontSize;
            SettingsNiOpacity.Value = _notifyIcons.Elements[_appSettings.NiIconsType].BgOpacity;
        }
    }
    ...
    private void ChangeTrayMonIconColor_ColorChanged(ColorPicker sender, ColorChangedEventArgs args)
    {
        ...
        if (SettingsNiGradientColorSwitcher.IsChecked == false)
            _notifyIcons.Elements[_appSettings.NiIconsType].Color =
    ...
    private void UpdateTrayMonIconGradientColor_Click(object sender, RoutedEventArgs e)
    {
        ...
        if (button != null)
        {
            if (button.IsChecked == true)
            {
                ...
                NiIconsColorPickerColorPicker.Color =
                    ParseColor(_notifyIcons.Elements[_appSettings.NiIconsType].SecondColor);
            }
            else
            {
    ...
    private void TrayMonResetDefaults_Click(object sender, object e)
    {
        ...
        _notifyIcons.Elements[_appSettings.NiIconsType].IsEnabled = true;
        _notifyIcons.Elements[_appSettings.NiIconsType].ContextMenuType = 1;
        _notifyIcons.Elements[_appSettings.NiIconsType].Color = "FF6ACF";
        _notifyIcons.Elements[_appSettings.NiIconsType].IconShape = 0;
    ...
    private void LoadRtssElements()
    {
        for (var i = 0; i < _rtssSettings.RtssElements.Count; i++)
        {
            ...
            if (toggleButton != null) toggleButton.IsChecked = _rtssSettings.RtssElements[i].UseCompact;

            // Применение значения CheckBox
            if (checkBox != null) checkBox.IsChecked = _rtssSettings.RtssElements[i].Enabled;

            // Применение значения TextBox
            if (textBox != null) textBox.Text = _rtssSettings.RtssElements[i].Name;
    ...
    private void GenerateAdvancedCodeEditor()
    {
        ...
        // "<C0=FFA0A0><C1=A0FFA0><C2=FC89AC><C3=fa2363><S1=70><S2=-50>\n" +
        for (var i = 0; i < colorLib.Count; i++) advancedCodeEditor.Append($"<C{i}={colorLib[i]}>");
        ...
        {
            if (!colorLib.Contains(color.Replace("#", ""))) colorLib.Add(color.Replace("#", ""));
        }
    }
    ...
    private void RtssChanged_Checked(object s, object e)
    {
        ...
        {
            if (textBox.Name == "RtssSakuOverclockPresetTextBox") _rtssSettings.RtssElements[2].Name = textBox.Text;

            if (textBox.Name == "RtssStapmFastSlowTextBox") _rtssSettings.RtssElements[3].Name = textBox.Text;

            if (textBox.Name == "RtssEdcThermUsageTextBox") _rtssSettings.RtssElements[4].Name = textBox.Text;

            if (textBox.Name == "RtssCpuClocksTextBox") _rtssSettings.RtssElements[5].Name = textBox.Text;

            if (textBox.Name == "RtssAvgCpuClockVoltTextBox") _rtssSettings.RtssElements[6].Name = textBox.Text;

            if (textBox.Name == "RtssApuClockVoltTempTextBox") _rtssSettings.RtssElements[7].Name = textBox.Text;

            if (textBox.Name == "RtssFrameRateTextBox") _rtssSettings.RtssElements[8].Name = textBox.Text;
        }

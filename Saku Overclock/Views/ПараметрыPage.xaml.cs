...
public sealed partial class ПараметрыPage
{
    private readonly IAppNotificationService
        _notificationsService = App.GetService<IAppNotificationService>(); // Уведомления приложения
    ...
    private readonly IAppSettingsService
        _appSettings = App.GetService<IAppSettingsService>(); // Все настройки приложения
    ...
    public ПараметрыPage()
    {
        ...
        if (_appSettings.Preset == -1) _isPremadePresetApplied = true;
    ...
    private void SlidersInit()
    {
        ...
        else
        {
            ...
            {
                _presetIndex = _appSettings.Preset;
                if (PresetCom.Items.Count >= _presetIndex + 1) PresetCom.SelectedIndex = _presetIndex + 1;
            }
        }
    ...
    private void LoadPresetsToComboBox()
    {
        foreach (var currPreset in _presetManager.Presets)
        {
            ...
            if (currPreset.PresetName.Contains("Preset_")) presetName = ГлавнаяPage.TryLocalize(presetName);
    ...
    private void MainInit(int index)
    {
        try
        {
            if (SettingsViewModel.VersionId != 5) // Если не дебаг. В дебаг версии отображаются все параметры
            {
                ...
                /*              A M 4  v 2    C P U                  */
                if (codenameGen == CodenameGeneration.Am4V2) DesktopCpu_AM4_HideUnavailableParameters();
                /*                 A M 5    C P U                    */
                if (codenameGen == CodenameGeneration.Am5) DesktopCpu_AM5_HideUnavailableParameters();
                ...
                for (var i = 0; i < _cpu.PhysicalCores; i++)
                {
                    ...
                    if (_cpu.CoreDisableMap.Length <= mapIndex) break;
                ...
                {
                    LogHelper.LogWarn(
                        "Curve Optimizer Disabled cores detection incorrect on that CPU. Using standard disabled cores detection method.");
                    ...
                    else
                    {
                        ...
                        if (cores == 0) Ccd1Expander.Visibility = Visibility.Collapsed;
                    }
                }
            }
            ...
            try
            {
                ...
                TrySetMaximum(_presetManager.Presets[index].SubsystemsSettings.MinimumIntegratedGraphicsFrequency.Value,
                    G9V);
                TrySetMaximum(_presetManager.Presets[index].SubsystemsSettings.MaximumIntegratedGraphicsFrequency.Value,
                    G10V);
                ...
                TrySetMaximum(_presetManager.Presets[index].FrequenciesSettings.IntegratedGraphicsFrequency.Value,
                    A10V);
            ...
            try
            {
                ...
                LoadUiOption(_presetManager.Presets[index].SubsystemsSettings.MinimumIntegratedGraphicsFrequency, G9,
                    G9V);
                LoadUiOption(_presetManager.Presets[index].SubsystemsSettings.MaximumIntegratedGraphicsFrequency, G10,
                    G10V);
                ...
                LoadUiOption(_presetManager.Presets[index].CurveOptimizerAdvancedOptions.CurveOptimizerPreferredMode,
                    CcdCoModeSel, CcdCoMode);
                LoadUiOption(_presetManager.Presets[index].CurveOptimizerOptions.CpuCurveOptimizerUndervoltingLevel, O1,
                    O1V);
                LoadUiOption(
                    _presetManager.Presets[index].CurveOptimizerOptions.IntegratedGpuCurveOptimizerUndervoltingLevel,
                    O2, O2V);

                LoadUiOption(_presetManager.Presets[index].CurveOptimizerAdvancedOptions.CurveOptimizerCores, 0, Ccd11,
                    Ccd11V);
                LoadUiOption(_presetManager.Presets[index].CurveOptimizerAdvancedOptions.CurveOptimizerCores, 1, Ccd12,
                    Ccd12V);
                LoadUiOption(_presetManager.Presets[index].CurveOptimizerAdvancedOptions.CurveOptimizerCores, 2, Ccd13,
                    Ccd13V);
                LoadUiOption(_presetManager.Presets[index].CurveOptimizerAdvancedOptions.CurveOptimizerCores, 3, Ccd14,
                    Ccd14V);
                LoadUiOption(_presetManager.Presets[index].CurveOptimizerAdvancedOptions.CurveOptimizerCores, 4, Ccd15,
                    Ccd15V);
                LoadUiOption(_presetManager.Presets[index].CurveOptimizerAdvancedOptions.CurveOptimizerCores, 5, Ccd16,
                    Ccd16V);
                LoadUiOption(_presetManager.Presets[index].CurveOptimizerAdvancedOptions.CurveOptimizerCores, 6, Ccd17,
                    Ccd17V);
                LoadUiOption(_presetManager.Presets[index].CurveOptimizerAdvancedOptions.CurveOptimizerCores, 7, Ccd18,
                    Ccd18V);
                LoadUiOption(_presetManager.Presets[index].CurveOptimizerAdvancedOptions.CurveOptimizerCores, 8, Ccd21,
                    Ccd21V);
                LoadUiOption(_presetManager.Presets[index].CurveOptimizerAdvancedOptions.CurveOptimizerCores, 9, Ccd22,
                    Ccd22V);
                LoadUiOption(_presetManager.Presets[index].CurveOptimizerAdvancedOptions.CurveOptimizerCores, 10, Ccd23,
                    Ccd23V);
                LoadUiOption(_presetManager.Presets[index].CurveOptimizerAdvancedOptions.CurveOptimizerCores, 11, Ccd24,
                    Ccd24V);
                LoadUiOption(_presetManager.Presets[index].CurveOptimizerAdvancedOptions.CurveOptimizerCores, 12, Ccd25,
                    Ccd25V);
                LoadUiOption(_presetManager.Presets[index].CurveOptimizerAdvancedOptions.CurveOptimizerCores, 13, Ccd26,
                    Ccd26V);
                LoadUiOption(_presetManager.Presets[index].CurveOptimizerAdvancedOptions.CurveOptimizerCores, 14, Ccd27,
                    Ccd27V);
                LoadUiOption(_presetManager.Presets[index].CurveOptimizerAdvancedOptions.CurveOptimizerCores, 15, Ccd28,
                    Ccd28V);
            }
            catch
            {
    ...
    {
        if (maximum > slider.Maximum) slider.Maximum = FromValueToUpperFive(maximum);
    }
    ...
    public static int FromValueToUpperFive(double value)
    {
        return (int)Math.Ceiling(value / 5) * 5;
    }
    ...
    private void CollectSearchItems()
    {
        ...
        foreach (var expander in expanders)
        {
            ...
            {
                var textBlocks = VisualTreeHelper.FindVisualChildren<TextBlock>(stackPanel)
                    .Where(tb => tb.FontSize - 15 == 0);
    ...
    private void ResetVisibility()
    {
        ...
        foreach (var expander in expanders)
        {
            ...
            foreach (var stackPanel in stackPanels)
            {
                ...
                if (adjacentGrid != null) adjacentGrid.Visibility = Visibility.Visible;
            }
        }
    }
    ...
    private void SuggestBox_OnTextChanged(AutoSuggestBox? sender, AutoSuggestBoxTextChangedEventArgs? args)
    {
            ...
            args == null)
        {
            ...
            if (SuggestBox.Text == string.Empty) FilterButtons_ResetButton_Click(null, null);
            ...
            foreach (var expander in expanders)
            {
                ...
                foreach (var stackPanel in arrayStackPanels)
                {
                    var textBlocks = VisualTreeHelper.FindVisualChildren<TextBlock>(stackPanel)
                        .Where(tb => tb.FontSize - 15 == 0);
                    var containsText = textBlocks.Any(tb =>
                        tb.Text.Contains(SuggestBox.Text.ToLower(), StringComparison.CurrentCultureIgnoreCase));
                    ...
                    var adjacentGrid = VisualTreeHelper.FindAdjacentGrid(stackPanel);
                    if (adjacentGrid != null) adjacentGrid.Visibility = stackPanel.Visibility;
                }
                ...
                if (!anyVisible) expander.IsExpanded = false;
            }
        }
    }
    ...
    private void FilterButton_Checked(object sender, RoutedEventArgs e)
    {
        ...
        if (SuggestBox.Text == string.Empty) ResetVisibility();
        ...
        foreach (var expander in expanders)
        {
            ...
            foreach (var stackPanel in arrayStackPanels)
            {
                var textBlocks = VisualTreeHelper.FindVisualChildren<FontIcon>(stackPanel)
                    .Where(tb => tb.FontSize - 15 == 0);
            ...
            foreach (var grid in arrayHelperGrids)
                if (savedArray.Contains(Grid.GetRow(grid)) && Grid.GetColumn(grid) == 1 &&
    ...
    private void PresetCOM_SelectionChanged(object sender, SelectionChangedEventArgs e)
    {
        try
        {
            ...
            if (PresetCom.SelectedIndex != -1) _appSettings.Preset = PresetCom.SelectedIndex - 1;
    ...
    private void SuggestBox_Loaded(object sender, RoutedEventArgs e)
    {
        ...
        foreach (var text in texts) text.Margin = new Thickness(12, 10, 0, 0);
        ...
        foreach (var content in contents)
        {
            ...
            foreach (var present in presents)
            {
                ...
                foreach (var text in texts1) text.Margin = new Thickness(0, 5, 0, 0);
            }
        }
    }
    ...
    private void G9_Checked(object sender, RoutedEventArgs e)
    {
        ...
        if (_presetIndex != -1)
        {
            _presetManager.Presets[_presetIndex].SubsystemsSettings.MinimumIntegratedGraphicsFrequency.IsEnabled =
                check;
    ...
    private void G10_Checked(object sender, RoutedEventArgs e)
    {
        ...
        if (_presetIndex != -1)
        {
            _presetManager.Presets[_presetIndex].SubsystemsSettings.MaximumIntegratedGraphicsFrequency.IsEnabled =
                check;
    ...
    private void CCD2_8_Checked(object sender, RoutedEventArgs e)
    {
        ...
        if (_presetIndex != -1)
        {
            _presetManager.Presets[_presetIndex].CurveOptimizerAdvancedOptions.CurveOptimizerCores.IsEnabled[15] =
                check;
    ...
    private void CCD2_7_Checked(object sender, RoutedEventArgs e)
    {
        ...
        if (_presetIndex != -1)
        {
            _presetManager.Presets[_presetIndex].CurveOptimizerAdvancedOptions.CurveOptimizerCores.IsEnabled[14] =
                check;
    ...
    private void CCD2_6_Checked(object sender, RoutedEventArgs e)
    {
        ...
        if (_presetIndex != -1)
        {
            _presetManager.Presets[_presetIndex].CurveOptimizerAdvancedOptions.CurveOptimizerCores.IsEnabled[13] =
                check;
    ...
    private void CCD2_5_Checked(object sender, RoutedEventArgs e)
    {
        ...
        if (_presetIndex != -1)
        {
            _presetManager.Presets[_presetIndex].CurveOptimizerAdvancedOptions.CurveOptimizerCores.IsEnabled[12] =
                check;
    ...
    private void CCD2_4_Checked(object sender, RoutedEventArgs e)
    {
        ...
        if (_presetIndex != -1)
        {
            _presetManager.Presets[_presetIndex].CurveOptimizerAdvancedOptions.CurveOptimizerCores.IsEnabled[11] =
                check;
    ...
    private void CCD2_3_Checked(object sender, RoutedEventArgs e)
    {
        ...
        if (_presetIndex != -1)
        {
            _presetManager.Presets[_presetIndex].CurveOptimizerAdvancedOptions.CurveOptimizerCores.IsEnabled[10] =
                check;
    ...
    private void O1_Checked(object sender, RoutedEventArgs e)
    {
        ...
        if (_presetIndex != -1)
        {
            _presetManager.Presets[_presetIndex].CurveOptimizerOptions.CpuCurveOptimizerUndervoltingLevel.IsEnabled =
                check;
    ...
    private void CCD_CO_Mode_Sel_Checked(object sender, RoutedEventArgs e)
    {
        ...
        if (_presetIndex != -1)
        {
            _presetManager.Presets[_presetIndex].CurveOptimizerAdvancedOptions.CurveOptimizerPreferredMode.IsEnabled =
                check;
    ...
    private async void Save_Click(object sender, RoutedEventArgs e)
    {
        try
        {
            if (!string.IsNullOrWhiteSpace(SavePresetN.Text))
            {
                ...
                await _presetManager.AddPresetAsync(new Preset { PresetName = SavePresetN.Text });
